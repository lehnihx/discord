...
fn run_command_action<'a>(
  ...
  Box::pin(async move {
    let Some(action) = ctx.command.custom_data.downcast_ref::<ActionFn>() else {
      let error = std::io::Error::new(std::io::ErrorKind::Other, "missing command action metadata");
    ...
    action(poise::Context::Application(ctx))
      .await
      .map_err(|error| poise::FrameworkError::new_command(poise::Context::Application(ctx), error))
  })
}
